    ...
    T: AsyncRead + AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn upgrade_inbound(self, mut io: T, _: Self::Info) -> Self::Future {
        async move {
            ...
            let event: GroupProtocol =
                bincode::deserialize(&packet).map_err(|err| std::io::Error::new(std::io::ErrorKind::Other, err))?;
            Ok(event)
        }
        .boxed()
    }
    ...
    T: AsyncWrite + AsyncRead + Send + Unpin + 'static,
{
    ...
    fn upgrade_outbound(self, mut io: T, _: Self::Info) -> Self::Future {
        async move {
            let bytes = bincode::serialize(&self).map_err(|err| std::io::Error::new(std::io::ErrorKind::Other, err))?;
