...
pub struct Group {
    ...
    events: VecDeque<NetworkBehaviourAction<GroupEvent, OneShotHandler<GroupProtocol, GroupProtocol, InnerMessage>>>,
                ...
                match action.action {
                    GroupActionType::Join(port, is_request) => {
                            ...
                            self.events.push_back(NetworkBehaviourAction::NotifyHandler {
                                ...
                                event: GroupProtocol {
                                    ...
                                    actions: vec![GroupAction {
                                        ...
                                        action: GroupActionType::Join(self.config.local_port, false),
                                    }],
                                    messages: Vec::new(),
                                },
                            });
                        ...
                        if !peers.contains(&peer_id) && self.peers.contains_key(&peer_id) {
                            ...
                            peers.push(peer_id);
                            self.events
                                .push_back(NetworkBehaviourAction::GenerateEvent(GroupEvent::Join {
                                    peer: peer_id,
                                    group: action.group,
                                }));
                        }
                    ...
                    GroupActionType::Leave => {
                        if let Some(pos) = peers.iter().position(|x| x == &peer_id) {
                            peers.remove(pos);
                            self.events
                                .push_back(NetworkBehaviourAction::GenerateEvent(GroupEvent::Leave {
                                    peer: peer_id,
                                    group: action.group,
                                }));
                        }
