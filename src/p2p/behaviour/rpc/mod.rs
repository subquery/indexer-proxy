...
impl Rpc {
    ...
    fn try_send_request(&mut self, peer: &PeerId, request: RequestProtocol) -> Option<RequestProtocol> {
...
impl NetworkBehaviour for Rpc {
    ...
    fn inject_dial_failure(&mut self, peer: Option<PeerId>, _: Self::ConnectionHandler, _: &DialError) {
    ...
    fn inject_event(&mut self, peer: PeerId, connection: ConnectionId, event: RpcHandlerEvent) {
        ...
        match event {
            RpcHandlerEvent::Response { request_id, response } => {
                let removed = self.remove_pending_inbound_response(&peer, connection, &request_id);
                debug_assert!(removed, "Expect request_id to be pending before receiving response.",);

                let message = RpcMessage::Response { request_id, response };
                self.pending_events
            ...
            RpcHandlerEvent::Request {
                ...
                let message = RpcMessage::Request { request_id, request };
                self.pending_events
                    .push_back(NetworkBehaviourAction::GenerateEvent(RpcEvent::Message {
                        peer,
                        message,
            ...
            RpcHandlerEvent::ResponseSent(request_id) => {
                ...
                self.pending_events
                    .push_back(NetworkBehaviourAction::GenerateEvent(RpcEvent::ResponseSent {
                        peer,
                        request_id,
                    }));
            }
