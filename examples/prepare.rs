...
async fn main() {
    ...
    let result: U256 = contracts["SQToken"]
        .query("balanceOf", (miner.address(),), None, Options::default(), None)
        .await
        .unwrap();
...
async fn register_indexer(web3: &Web3<Http>, contract: &Contract<Http>, sk: &SecretKey, amount: u128) {
    ...
    let fn_data = contract
        ...
        .and_then(|function| function.encode_input(&(U256::from(amount), [0u8; 32], U256::from(0i32)).into_tokens()))
        .unwrap();
